...
class TimeMatrix(np.ndarray):
    ...
    # def cleanUp(self, i, duration):
    #     if duration>len(self[0])-1:
    #         raise ValueError(f"duration exceeds length of the day! {duration} > {len(self[0])-1}")
    #     for j in range (1, len(self[i])):
    #         if (self[i][j] != 0) and (not vineRight(self[i], j, duration)):
    #             print (f"cleaning up {self[i][j]}: {i} {j}")
    #             self[i][j] = 0
        # return self
...
class TimeVortex():
    def __init__(self, time_matrix=TimeMatrix([]), rowTimeLocked = -1, generate_data=False):
        ...
        #         self.data[item]["duration"] = random.randint(1, 5)
        #         self.data[item]["priority"] = random.randint(1, 5)
        if generate_data:
    ...
    def sortByDensityBelowRow(self, row):
        for i in range (row+1, len(self.matrix)):
            ...
            for j in range(i, len(self.matrix)):  # Iterate through rows below `row`
                # print (f"{j, [self.matrix[j][0]]}")
                # if self.matrix.countRow(j) == 0:
                #     print ("met 0s")
                #     continue
...
#             print (index)
#             print (row)
#             print (self.data[row[index]])
#             for j in range(1, len(row)):
#                 grouping += str(row[j]*self.data[row[0]]['priority']) + " "

#             string += f"{index} | {row[0]}: {grouping} \n"
...
class TardisUI:
    ...
    def create_event_card(self, row, event_id, event_data):
        ...
        # detail_text = f"Длительность: {event_data['duration']} | Приоритет: {priority_text}"
...
# if __name__ == "__main__":
#     # sample_matrix = TimeMatrix([timeString(), timeString(), timeString(), timeString(), timeString()])
#     vortex = TimeVortex(TimeMatrix([]))
#     root = tk.Tk()
#     root.geometry("445x600")
#     app = TardisUI(root, vortex)
